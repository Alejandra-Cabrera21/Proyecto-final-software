...
export default function CreateSurvey() {
  ...
  const updateQuestion = (i, field, value) => {
    ...
    if (field === 'type' && value !== 'likert' && value !== 'short_text') updated[i].options = ['', '']
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api('/surveys', { method: 'POST', body: JSON.stringify(payload) }, token)
      navigate('/')
  ...
  return (
    <div className="page" style={{ maxWidth: 680 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '1.5rem' }}>
        ...
        <h1 style={{ fontSize: 18, fontWeight: 600 }}>Nueva encuesta</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ marginBottom: 16 }}>
          <div className="field">
            ...
            <input className="input" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ej: Encuesta de satisfacción" required />
          </div>
          <div className="field">
            <label className="label">Descripción</label>
            <textarea className="input" value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="Describe el objetivo de la encuesta" style={{ resize: 'vertical' }} />
          </div>
        ...
        {questions.map((q, i) => (
          <div key={i} className="card" style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              ...
              {questions.length > 1 && <button type="button" className="btn btn-sm" onClick={() => removeQuestion(i)} style={{ color: 'var(--danger)', borderColor: 'var(--danger)' }}>Eliminar</button>}
            </div>
            <div className="field">
              ...
              <div>
                ...
                {(q.options || []).map((opt, oi) => (
                  <div key={oi} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                    ...
                    {(q.options || []).length > 2 && <button type="button" className="btn btn-sm" onClick={() => { const updated = [...questions]; updated[i].options = updated[i].options.filter((_, idx) => idx !== oi); setQuestions(updated) }}>×</button>}
                  </div>
                ))}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? <span className="spinner" /> : 'Crear encuesta'}
          </button>
        </div>
      </form>
    </div>
